...
public class ToolPaint extends Application {
    ...
    private boolean translucentFill;
    private boolean translucentStroke;
     ...
     */
    private enum Tool { LINE, RECT, OVAL, FILLED_RECT, FILLED_OVAL, CURVE, SMUDGE, ERASE, STROKED_FILLED_OVAL, STROKED_FILLED_RECT }

    private Tool currentTool = Tool.CURVE;  // The current drawing tool.

    private Color currentFillColor = Color.BLACK;  // The current drawing color.

    private Color currentStrokeColor = Color.BLACK;  // The current drawing color.
     ...
     */
    private void putRect(GraphicsContext g, boolean stroked,boolean filled, int x1, int y1, int x2, int y2) {
        if (x1 == x2 || y1 == y2)
            return;
        ...
        int strokeLocation;
        if(strokeWidth%2==0){
            strokeLocation = (strokeWidth-1)/2+1;
        }else{
            strokeLocation = strokeWidth/2;
        }

        if (filled && stroked){
            g.fillRect(x+strokeLocation,y+strokeLocation,w-strokeLocation*2,h-strokeLocation*2);
     ...
     */
    private void putOval(GraphicsContext g, boolean stroked, boolean filled, int x1, int y1, int x2, int y2) {
        if (x1 == x2 || y1 == y2)
            return;
        int x = Math.min(x1,x2);    // get upper left corner, (x,y)
        int y = Math.min(y1,y2);
        ...
        if(filled && stroked){
            g.fillOval(x+strokeLocation,y+strokeLocation,w-strokeLocation*2,h-strokeLocation*2);
    ...
    private MenuBar makeMenuBar() {
        ...
        Menu backgroundColorMenu = new Menu("Canvas");
        Menu checkMenu = new Menu("Translucency");
        Menu fillColorMenu = new Menu("Fill Color");
        Menu toolMenu = new Menu("Tool");
        Menu strokeWidthMenu = new Menu("Stroke Width");

        menubar.getMenus().add(fileMenu);
        menubar.getMenus().add(backgroundColorMenu);
        menubar.getMenus().add(strokeColorMenu);
        menubar.getMenus().add(fillColorMenu);
        menubar.getMenus().add(checkMenu);
        menubar.getMenus().add(toolMenu);
        menubar.getMenus().add(strokeWidthMenu);


        CheckMenuItem translucentFillCheckbox  = new CheckMenuItem("Translucent Fill");
        checkMenu.getItems().add(translucentFillCheckbox);
        translucentFillCheckbox.setOnAction(e-> translucentFill =translucentFillCheckbox.isSelected());
        translucentFillCheckbox.setAccelerator( KeyCombination.valueOf("Shortcut+T") );

        CheckMenuItem translucentStrokeCheckbox  = new CheckMenuItem("Translucent Stroke");
        checkMenu.getItems().add(translucentStrokeCheckbox);
        translucentStrokeCheckbox.setOnAction(e-> translucentStroke =translucentStrokeCheckbox.isSelected());
        translucentStrokeCheckbox.setAccelerator( KeyCombination.valueOf("Shortcut+Shift+T") );

        MenuItem openImage = new MenuItem("Load Image...");
        openImage.setOnAction( e -> doOpenImage() );
        openImage.setAccelerator( KeyCombination.valueOf("Shortcut+Shift+O") );
        ...
        ToggleGroup fillColorGroup = new ToggleGroup();
        for (int i = 0; i < colors.length; i++) {
            RadioMenuItem item = new RadioMenuItem("Draw with " + colorNames[i]);
            item.setUserData(colors[i]); // Stash the actual color in the menu item's UserData
            item.setToggleGroup(fillColorGroup);
            fillColorMenu.getItems().add(item);
            if (i == 0)
                item.setSelected(true);  // Initially selected color is black.
        }

        RadioMenuItem strokeCustomColor = new RadioMenuItem("Custom Drawing Color...");
        strokeCustomColor.setToggleGroup(strokeColorGroup);
        strokeCustomColor.setOnAction( e -> {
            ...
            strokeCustomColor.setSelected(true);
            Color c = SimpleDialogs.colorChooser(currentStrokeColor, "Select a Color to Use For Drawing");
            if (c != null)  // c is null if user cancels the dialog
                currentStrokeColor = c;
        });
        strokeColorMenu.getItems().add(strokeCustomColor);
        strokeColorGroup.selectedToggleProperty().addListener( e -> {
            ...
            Toggle t = strokeColorGroup.getSelectedToggle();  // the selected RadioMenuItem
            if (t != null && t != strokeCustomColor) {
        ...
        RadioMenuItem fillCustomColor = new RadioMenuItem("Custom Drawing Color...");
        fillCustomColor.setToggleGroup(fillColorGroup);
        fillCustomColor.setOnAction( e -> {
            // For the custom color selection, use a dialog box to get an
            // arbitrary color from the user.  This has to be done in an
            // ActionEvent handler, since it needs to happen even if the user selects
            // Custom Drawing Color when it is already selected.  (In that case,
            // the selected toggle does not change.
            fillCustomColor.setSelected(true);
            Color c = SimpleDialogs.colorChooser(currentFillColor, "Select a Color to Use For Drawing");
            if (c != null)  // c is null if user cancels the dialog
                currentFillColor = c;
        });
        fillColorMenu.getItems().add(fillCustomColor);
        fillColorGroup.selectedToggleProperty().addListener( e -> {
            // Sets the color, when one of the standard colors is selected.
            // This does not handle the Custom Color option.  Note that
            // when the user chooses a new radio menu item, the selected toggle
            // first changes to null as the old menu item is deselected, then
            // changes to the newly selected menu item.
            Toggle t = fillColorGroup.getSelectedToggle();  // the selected RadioMenuItem
            if (t != null && t != fillCustomColor) {
                // The color associated with this menu item was stashed
                // in the UserData of the menu item, t.
                currentFillColor = (Color)t.getUserData();
            }
        });

        backgroundColorMenu.getItems().add( new SeparatorMenuItem() );
        ...
        backgroundColorMenu.getItems().add(clear);
        ...
        fill.setOnAction( e -> {  // Fill main canvas with current drawing color, but
            ...
            canvasGraphics.setFill(currentFillColor);
            canvasGraphics.fillRect(0,0,canvas.getWidth(),canvas.getHeight());
        });
        backgroundColorMenu.getItems().add(fill);
        backgroundColorMenu.getItems().add(new SeparatorMenuItem());
        ...
        backgroundColorMenu.getItems().add(setBG);
        ...
        Tool[] tools = { // The available tools in the order they appear in the menu.
                ...
                Tool.FILLED_OVAL, Tool.STROKED_FILLED_RECT,Tool.STROKED_FILLED_OVAL,Tool.SMUDGE, Tool.ERASE };
        String[] toolNames = { // Names for the tools, used as text in the menu items.
                "Curve", "Line", "Rectangle", "Oval", "Filled Rectangle",
                "Filled Oval", "Stroked Filled Rectangle", "Stroked Filled Oval","Smudge", "Erase", };
        ...
        for (int i = 0; i < tools.length; i++) {
            ...
            if (i == 0 || i == 7)  // Separators before and after the shape tools.
        ...
        toolMenu.getItems().get(0).setAccelerator( KeyCombination.valueOf("P") );
        toolMenu.getItems().get(2).setAccelerator( KeyCombination.valueOf("L") );
        toolMenu.getItems().get(3).setAccelerator( KeyCombination.valueOf("R") );
        toolMenu.getItems().get(4).setAccelerator( KeyCombination.valueOf("C") );
        toolMenu.getItems().get(5).setAccelerator( KeyCombination.valueOf("Shift+R") );
        toolMenu.getItems().get(6).setAccelerator( KeyCombination.valueOf("Shift+C") );
        toolMenu.getItems().get(7).setAccelerator( KeyCombination.valueOf("Shortcut+Shift+R") );
        toolMenu.getItems().get(8).setAccelerator( KeyCombination.valueOf("Shortcut+Shift+C") );




        ToggleGroup strokeWidthGroup = new ToggleGroup();
        int[] strokeWidths = new int[]{1,2,3,4,5,10,15,20,30,40,50};
        String[] strokeWidthNames = new String[]{"1","2","3","4","5","10","15","20","30","40","50"};
        for (int i = 0; i < strokeWidths.length; i++) {
            RadioMenuItem item = new RadioMenuItem(strokeWidthNames[i]);
            item.setUserData(strokeWidths[i]);
            item.setToggleGroup(strokeWidthGroup);
            strokeWidthMenu.getItems().add(item);
            if (i == 0)
                item.setSelected(true);
        }
        strokeWidthGroup.selectedToggleProperty().addListener( e -> {
            Toggle t = strokeWidthGroup.getSelectedToggle();  // The selected RadioMenuItem
            if (t != null){
                strokeWidth = (int)t.getUserData();
            }
        });
    ...
    private void doUndo(){
        canvasGraphics.drawImage( previousAction, 0, 0, canvas.getWidth(), canvas.getHeight() );
    }


    private Color getTranslucentColor(Color originalColor){
    ...
    private void mousePressed(MouseEvent evt) {
        previousAction = canvas.snapshot(null,null);
        startX = prevX = currentX = (int)evt.getX();
        startY = prevY = currentY = (int)evt.getY();
        Color translucentFillColor;
        Color translucentStrokeColor;
        dragging = true;
        if(translucentFill){
            translucentFillColor = getTranslucentColor(currentFillColor);
            canvasGraphics.setFill(translucentFillColor);
            overlayGraphics.setFill(translucentFillColor);
        }else{
            canvasGraphics.setFill(currentFillColor);
            overlayGraphics.setFill(currentFillColor);
        }
        if(translucentStroke){
            translucentStrokeColor = getTranslucentColor(currentStrokeColor);
            canvasGraphics.setStroke(translucentStrokeColor);
            overlayGraphics.setStroke(translucentStrokeColor);
        }else{
            canvasGraphics.setStroke(currentStrokeColor);
            overlayGraphics.setStroke(currentStrokeColor);
        }

        canvasGraphics.setLineWidth(strokeWidth);
        overlayGraphics.setLineWidth(strokeWidth);
